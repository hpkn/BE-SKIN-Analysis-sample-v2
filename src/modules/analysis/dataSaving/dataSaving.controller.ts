import { Controller, Body, Get, Post, UseInterceptors, UploadedFiles, Res,  } from '@nestjs/common';
...
export class DataSavingController {
    ...
    async getBatchId(@Body() data: any, @Res() res: Response) {
        try{
            let {customer_id, deviceOS, latitude, longitude, deviceModel, hpFlag} = data
            let metadata = {'deviceOS': deviceOS, 'latitude': latitude, 'longitude': longitude, 'deviceModel': deviceModel, "hpFlag": hpFlag}
            let insertObjet = JSON.stringify(metadata);
            const insert = await this.dataSaving.insertInAnalysis(customer_id, insertObjet)
    ...
    async saveData(
        ...
        if(files?.ppl) {
            if(files?.ppl[0]) {
                ...
                const imageArg = this.fileUpload.getImageArgs('front_ppl', "/images/original/");
                let imageType = await this.dataSaving.getImage(typeOfImage)
                let args = typeOfImage
                await this.fileUpload.uploadImage(files?.ppl[0].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
            }
            
            if(files?.ppl[1]){
                typeOfImage = 'left_ppl'
                const imageArg = this.fileUpload.getImageArgs('left_ppl', "/images/original/");
                let imageType = await this.dataSaving.getImage(typeOfImage)
                let args = typeOfImage
                await this.fileUpload.uploadImage(files?.ppl[1].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
            }  
           
            if(files?.ppl[2]){
                typeOfImage = 'right_ppl'
                const imageArg = this.fileUpload.getImageArgs(typeOfImage, "/images/original/");
                let imageType = await this.dataSaving.getImage(typeOfImage)
                let args = typeOfImage
                await this.fileUpload.uploadImage(files?.ppl[2].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
            }
        }
        ...
        if(files.xpl){
            if(files.xpl[0]) {
                ...
                let imageType = await this.dataSaving.getImage(typeOfImage)
                let args = typeOfImage
                await this.fileUpload.uploadImage(files.xpl[0].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
            }
            ...
            if(files.xpl[1]){
                ...
                let args = typeOfImage
                const imageArg = this.fileUpload.getImageArgs(typeOfImage, "/images/original/");
                await this.fileUpload.uploadImage(files.xpl[1].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
            }  
            ...
            if(files.xpl[2]){
                ...
                const imageArg = this.fileUpload.getImageArgs(typeOfImage, "/images/original/");
                let args = typeOfImage
                await this.fileUpload.uploadImage(files.xpl[2].buffer, imageArg.sys_url);
                this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
        ...
        if(type.toLowerCase() === 'redness') {
            ...
            let frontfileArg = this.fileUpload.getMaskArgs('redness', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontfileArg.sys_url);
            let leftfileArg = this.fileUpload.getMaskArgs('redness', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[1].buffer, leftfileArg.sys_url);
            let rightfileArg = this.fileUpload.getMaskArgs('redness', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[2].buffer, rightfileArg.sys_url);

            const images = {
                front: {
                    url: frontfileArg.url, sys_url: frontfileArg.sys_url, hash: frontfileArg.hash
                },
                left: {
                    url: leftfileArg.url, sys_url: leftfileArg.sys_url, hash: leftfileArg.hash
                },
                right: {
                    url: rightfileArg.url, sys_url: rightfileArg.sys_url, hash: rightfileArg.hash
                }
            }
            ...
            this.dataSaving.insertInMesurement(batch_id, JSON.stringify(args), measureId)
            ...
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontWArg.sys_url);
            let frontGArg = this.fileUpload.getMaskArgs('oiliness', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[1].buffer, frontGArg.sys_url);

            const images = {
                frontW: {
                    url: frontWArg.url, sys_url: frontWArg.sys_url, hash: frontWArg.hash
                },
                frontG: {
                    url: frontGArg.url, sys_url: frontGArg.sys_url, hash: frontGArg.hash
                }
            }
            ...
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontArg.sys_url);
   

            const images = {
                front: {
                    url: frontArg.url, sys_url: frontArg.sys_url, hash: frontArg.hash
                },
            }
            ...
            if(files.uvl_impurities) {
                if(files.uvl_impurities[0]) {
                    ...
                    let imageType = await this.dataSaving.getImage(typeOfImage)
                    let args = typeOfImage
                    await this.fileUpload.uploadImage(files.uvl_impurities[0].buffer, imageType.sys_url);

                    this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
                }
                const images = {
                    front: {
                        url: frontArg.url, sys_url: frontArg.sys_url
                    },
                }
                const args = {images: images, score: score, rawValues: rawValues}
                const measureId = await this.dataSaving.getMeasurmant(type.toLowerCase())
           ...
           const score = {
            ...
            let frontfileArg = this.fileUpload.getMaskArgs('wrinkle', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontfileArg.sys_url);
            let leftfileArg = this.fileUpload.getMaskArgs('wrinkle', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[1].buffer, leftfileArg.sys_url);
            let rightfileArg = this.fileUpload.getMaskArgs('wrinkle', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[2].buffer, rightfileArg.sys_url);
            ...
            if(files.uvl) {
                if(files.uvl[0]) {
                    ...
                    await this.fileUpload.uploadImage(files.uvl[0].buffer, imageArg.sys_url);
                    this.dataSaving.insertImage(batch_id, imageArg.url, imageArg.sys_url, imageArg.hash, imageType, JSON.stringify({args}))
                }
                ...
                if(files.uvl[1]){
                    ...
                    let args = typeOfImage
                    const imageArg = this.fileUpload.getImageArgs(typeOfImage, "/images/original/");
                    await this.fileUpload.uploadImage(files.uvl[1].buffer, imageArg.sys_url);
            ...
            let frontfileArg = this.fileUpload.getMaskArgs('hyperpigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontfileArg.sys_url);
            let leftfileArg = this.fileUpload.getMaskArgs('hyperpigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[1].buffer, leftfileArg.sys_url);
            let rightfileArg = this.fileUpload.getMaskArgs('hyperpigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[2].buffer, rightfileArg.sys_url);
            ...
            let frontfileArg = this.fileUpload.getMaskArgs('pigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[0].buffer, frontfileArg.sys_url);
            let leftfileArg = this.fileUpload.getMaskArgs('pigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[1].buffer, leftfileArg.sys_url);
            let rightfileArg = this.fileUpload.getMaskArgs('pigmentation', "/images/mask/");
            await this.fileUpload.uploadImage(files.masks[2].buffer, rightfileArg.sys_url);
    ...
    async saveMetaResult(@Body() body: any, @Res() res : Response){
        ...
        const response = await this.dataSaving.insertInMesurement(batch_id, JSON.stringify({args}), getMeasurmant)
    ...
    async computationSave(@Body() body: any, @Res() res : Response){
        let { batch_id, sensitivityQ, sensititivityA, wrinkleQ, wrinkleA, spotsQ, spotsA, oilinessQ, oilinessA, wrinkles_score, pig_score, redness_score, oiliness_score} = body
        ...
        await this.dataSaving.insertInMesurement(batch_id, JSON.stringify({args}), getMeasurmant)
        return res.status(201).send({
            status: 200,
            service: "CFA Offline Analysis Data saving",
            message: 'Data saved to the cloud'
