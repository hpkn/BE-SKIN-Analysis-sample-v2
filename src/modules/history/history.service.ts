...
export class HistoryService {
    ...
    async getAnalysisImage(batch_id: number) {
        const batchRes = await this.database.executeQuery(` 
            ...
            FROM analysis as an
            INNER JOIN (SELECT DISTINCT type_image_id, url, batch_id FROM analysis) as img ON img.batch_id = an.batch_id
    ...
    async getCustomerAnalysis(customer_id: number, offset: number, limit: number) {
        const count = await this.database.executeQuery(
            `SELECT COUNT(*) FROM analysis WHERE customer_id = ${customer_id}`,
        );
